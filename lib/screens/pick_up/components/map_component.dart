...
import 'package:taxi_arf/services/google_map_service.dart';
...
class _MapComponentState extends State<MapComponent> {
  ...
  void addMarker({double iconSize = 100.0}) async {
    final BitmapDescriptor customIcon = await getResizedIcon(
      'assets/marker/marker-pin-white.png', 
      iconSize, 
    );
  ...
  void addDraggableMarker() async {
    final BitmapDescriptor customIcon = await getResizedIcon(
      'assets/marker/marker-pin-orange.png', 
      100,
    );
  ...
  void addRiderMarker() async {
    ...
    if (myLocation != null) {
      ...
      moveMarkerAlongRoute(
          route, const MarkerId('riderMarker')); 
    }
  }
  ...
  Future<void> moveMarkerAlongRoute(
    ...
    for (int i = 0; i < route.length - 1; i++) {
      ...
      final int steps = segmentDuration ~/ 16; 
      ...
      child: GoogleMap(
        ...
        onCameraMove: (CameraPosition position) {
          setState(() {
            ...
            _updateMarkerRotation(_mapRotation); 
          });
        },
      ),
    );
